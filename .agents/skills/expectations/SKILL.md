...
uv run thesis analysis save-guidance <TICKER> \
  --revenue-low N --revenue-high N --source-filing 10-Q --source-date YYYY-MM-DD
```
...
Guidance 추출은 Codex 판단이며 Python은 저장과 이전 snapshot 대비 `RAISED/MAINTAINED/LOWERED` 계산만 담당한다. 컨센서스·revision·guidance만으로 BUY/SELL을 판정하지 않는다.
