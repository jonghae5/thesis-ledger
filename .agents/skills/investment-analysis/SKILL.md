...
description: 기업 데이터·기대치·밸류에이션·market pulse를 종합해 투자 메모와 thesis 변화를 판단한다. "지금 살 만한가", "지난 분석 이후 뭐가 달라졌어" 같은 질문에 사용.
---
...
이 MCP는 컨센서스·revision을 제공하지 않고 연간 재무에 실제 filing date를 붙이지 않으므로 현재 한국 종목은 `can_research=true`, `can_decide=false` 범위다. `investment_analysis`나 `fundamental_snapshots`에 저장하지 않고, 방향성 판단·기대수익률·confidence를 만들지 않는다. 최신 뉴스와 catalyst는 필요한 경우 `market-pulse` Skill로 별도 확인하며, 포트폴리오 수량·평균단가·자산 같은 개인 정보는 MCP에 전달하지 않는다.
...
3. 종합 업데이트, 최신 뉴스, catalyst 또는 실적 이후 변화를 다룰 때는 `market-pulse` Skill의 뉴스 모드로 웹 원문을 확인한다. Reddit 민심이나 retail narrative가 질문에 포함되거나 thesis에 중요한 과열·불신이 관측될 때만 Reddit 모드도 사용한다. 좁은 재무·valuation 질문에는 이 절차를 자동으로 확장하지 않는다.
...
- Reddit sentiment와 discussion momentum은 supplemental `LLM_INFERENCE`로만 사용한다. mention volume과 sentiment를 구분하고, 민심만으로 expectation anchor·variant perception·thesis 상태를 확정하지 않는다.
