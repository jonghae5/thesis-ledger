...
설정은 `config/investor-policy.example.json`을 복사해 작성한다. 보유정보는 영구 저장돼 있다고 가정하지 않고 사용자가 현재 조사에 제공한 수량·평균단가·투자 가능 자산을 사용한다. 현재 보유정보가 없으면 필요한 값만 묻고, 정책 상한과 실제 비중을 나란히 비교해 답한다.
